...
Data pipeline for ingesting massive.com US stocks and options market data into hive-partitioned Parquet files, queryable through DuckDB.

## What It Does

Downloads flat files from Massive's S3 bucket via rclone, converts CSV.gz aggregates (daily and minute-level) into sorted, hive-partitioned Parquet, fetches missing options contract metadata from the Massive REST API, and exposes everything as DuckDB in-memory views.

### Data Coverage

- **Stocks**: Daily and minute aggregates (SIP)
- **Options**: Daily and minute aggregates (OPRA)
- **Contracts**: Options contract metadata (Massive REST API)
...
- Python 3.x
- [rclone](https://rclone.org/) configured with a remote named `s3massive` pointing at the massive.com S3 flat file bucket (see [Massive flat files documentation](https://massive.com/flat-files))
- A [massive.com](https://massive.com/) API key (for contract metadata)
...
# 2. Run the printed rclone command to download from Massive S3
...
```
{data_root}/
├── downloads/                # Raw CSV.gz from Massive S3
│   ├── us_stocks_sip/
...
- **massive.com** — market data source (S3 flat files + REST API)
